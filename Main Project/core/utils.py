...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, classes=read_class_names(cfg.YOLO.CLASSES)):
    ...
    for i in range(num_boxes):
        ...
        class_ind = class_ind + 7 # bb colors
        ...
        if info:
            ...
            print(" {} Object found | Prediction Confidence: {:.2f}% | At Coordinate : Box Coordinates (Xmin, Ymin, Xmax, Ymax): {}, {}, {}, {} ".format(class_name, score*100, coor[0], coor[1], coor[2], coor[3]))

        if show_label:
            
            bbox_mess = '{}: {:.1f}%'.format(class_name, score*100)
        ...
        if counted_classes != None:
            height_ratio = int(image_h / 28)
            offset_value = 63
            
            for key, value in counted_classes.items():
                cv2.putText(image, "Total Number of Object Detected : {}".format(num_boxes), (5,38),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,(208,64,224),2 )
                cv2.putText(image, "{}s Detected: {}".format(key, value), (5, offset_value),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (64,224,208), 2)
                offset_value += height_ratio
    return image
